...
class FeatureCreator:
    ...
    def rec_loc(self, user_input_cuisine, user_input_price):
        self.z['Cusines_all'] = self.z['Cusines_all'].apply(str.lower)
        filterd_data = self.z.loc[(self.z['Cusines_all']==user_input_cuisine) & (self.z['price_for_one']==user_input_price)]

        if filterd_data.empty:
            return "Sorry, we couldn't find any restaurants matching your preferences."
        else:
            best_location = filterd_data.loc[filterd_data['Rating'].idxmax()]['location']
            return (f"The best location for {user_input_cuisine} cuisine within your budget is {best_location}.")
