...
def showRecommendaion():
    ...
    if pref_price > suggested_price:
        processed_suggested_price=round(suggested_price+(pref_price-suggested_price)*0.5,2)
    else:
        processed_suggested_price=round(suggested_price,2)
    
    res_loc = featureCreatorObj.rec_loc(selected_cusine_name,pref_price)

    return render_template('index.html',loc=prefferd_loc,price=pref_price,cuisine=selected_cusine_name,avg_price=avg_price,cusines=cusines,pop_res=pop_res,pop_res_cusine=pop_res_cusine,pop_res_prefCus_wise=pop_res_prefCus_wise,rec_price=suggested_price,pro_rec_price=processed_suggested_price,rec_loc=res_loc)
